...
import { syncWidgetData, syncTrackerList } from "@/services/widget-sync"
...
export function DailyPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      fetchTrackers().then(t => { syncAlarms(t); syncTrackerList(t) }).catch(() => {})
    } catch { /* Keep existing data on error — don't reset to empty */ } finally { setLoading(false) }
  }, [dateStr])
